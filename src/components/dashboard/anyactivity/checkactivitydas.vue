  ...
  >
    <template v-slot:top>
      <v-toolbar flat color="white">
        <v-toolbar-title>ตรวจสอบหลักฐาน : Pending </v-toolbar-title>
    ...
    <template v-slot:item.action="{ item }">
      ...
      <v-btn color="blue darken-1" text :href="'http://172.16.1.49:8083/'+ item.img" target="_bank">
      <v-icon medium class="mr-2" >
        mdi-file-image
      </v-icon>
      </v-btn>
...
export default {
  ...
  methods: {
    ...
    async aprove(item) {
      ...
      const response = await anyactivity.approvedataanyactivity({
          uuidpayment:item.uuid_any_activity_check,
  ...
  async mounted() {
    try {
      const response = await anyactivity
        .getdataanyactivity(this.$store.getters.token)
        .then(res => {
          return res;
        });
