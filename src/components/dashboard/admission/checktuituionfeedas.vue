  ...
  >
    <template v-slot:top>
      <v-toolbar flat color="white">
        <v-toolbar-title>ค่าเทอม : Pending </v-toolbar-title>
...
export default {
  ...
  methods: {
    ...
    async aprove(item) {
      ...
      const response = await getdataexam.putdatatuitionfee({uuid:item.uuid},this.$store.getters.token)
      .then(res=>{
        return res;
      })
      .catch(error=>{
  ...
  async mounted() {
    try {
      const response = await getdataexam
        .getdatatuitionfee(this.$store.getters.token)
        .then(res => {
          return res;
        });
