  ...
  >
    <template v-slot:top>
      <v-toolbar flat color="white">
        <v-toolbar-title>ค่าสมัครสอบ : Pending </v-toolbar-title>
...
export default {
  ...
  methods: {
    ...
    async aprove(item) {
      ...
      const response = await getdataexam.putdataexam({uuid:item.uuid},this.$store.getters.token)
      .then(res=>{
        return res;
      })
      .catch(error=>{
  ...
  async mounted() {
    try {
      const response = await getdataexam
        .getdataexam(this.$store.getters.token)
        .then(res => {
          return res;
        });
